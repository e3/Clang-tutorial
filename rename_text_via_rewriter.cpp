...
public:
	...
	bool VisitDecl(Decl *d)
	{
		...
		if(vd->getNameAsString().compare("rename_me") != 0) 
			return true;

		std::string anyhow("new_name");
		StringRef s (anyhow);
		TheRewriter.ReplaceText(vd->getLocation(),s);
...
int main (int argc, char* argv[])
{
    ...
    const FileEntry *pFile = 
		ci.getFileManager().getFile("input_rename_text_via_rewriter.cpp");
